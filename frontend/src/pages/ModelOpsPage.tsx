...
export function ModelOpsPage() {
  ...
  const releaseLabel = status.reportStatus === 'BENCHMARK_PASS'
    ? 'Benchmark 通过，但只可进入客户数据准备'
    : status.reportStatus === 'CUSTOMER_PILOT_PASS'
      ? '具备申请 Pilot approval 资格'
      : '证据不足或 gate 失败，禁止批准/激活'
  return (
    <div className="page modelops-page">
      <div className="page-heading">
        <div><span className="page-kicker">Deployment Pack · ModelOps 证据</span><h1>模型与发布边界</h1><p>把配置契约、数据来源、模型候选状态和评测证据放在同一张工作台上。演示 Pack ACTIVE 不代表模型已经获准生产发布。</p></div>
        <button className="secondary-button" onClick={() => void load()}><RefreshCw size={15} />刷新证据</button>
      </div>
      ...
      <div className="governance-top-grid">
        <section className="work-panel model-identity-panel">
          <div className="panel-heading compact-heading"><div><span className="section-index">01</span><h2>当前模型契约</h2></div><FileCheck2 size={18} /></div>
        ...
        <section className="work-panel release-gate-panel">
          <div className="panel-heading compact-heading"><div><span className="section-index">02</span><h2>发布门禁</h2></div><ShieldAlert size={19} /></div>
          ...
          <button className="secondary-button" disabled={!status.activationAllowed} title={status.activationAllowed ? '需具名审批后激活' : '必须先完成客户数据 provenance、门槛和审批'}>激活模型</button>
        </section>
      </div>

      <section className="work-panel evidence-panel">
        <div className="panel-heading compact-heading"><div><span className="section-index">03</span><h2>评测证据</h2></div><span className="panel-note">报告更新 {formatDateTime(status.reportGeneratedAt, true)}</span></div>
      ...
      <div className="governance-two-col">
        <section className="work-panel threshold-panel">
          <div className="panel-heading compact-heading"><div><span className="section-index">04</span><h2>策略阈值</h2></div><span className="panel-note">配置来自当前 Pack</span></div>
          {deployment ? <>
        ...
        <section className="work-panel limits-panel">
          <div className="panel-heading compact-heading"><div><span className="section-index">05</span><h2>限制与下一步</h2></div><span className="panel-note">发布建议</span></div>
